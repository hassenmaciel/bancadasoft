...
describe("Asaas admin diagnostic", () => {
  ...
  it("normalizes invisible whitespace in the Production API key before constructing the header", async () => { const fetcher=vi.fn(async(_url:string,init?:RequestInit)=>{expect((init?.headers as Record<string,string>).access_token).toBe("test-only-key");return new Response(null,{status:200})});const result=await testAsaasConnection({...validEnv,ASAAS_API_KEY:"  test-only-key\r\n"},fetcher);expect(fetcher).toHaveBeenCalledOnce();expect(result).toMatchObject({ok:true,httpStatus:200,reason:null}); });

...
export type PaymentCancelResult="CANCELLED"|"ALREADY_GONE"|"REJECTED"|"UNCERTAIN";
export interface PaymentProvider{readonly code:string;cancelPayment?(externalPaymentId:string):Promise<PaymentCancelResult>;createPixPayment(input:PixPaymentInput):Promise<PixPaymentResult>;getPixPaymentDetails?(externalPaymentId:string):Promise<PixPaymentDetails>;getPaymentStatus(externalPaymentId:string):Promise<PaymentStatusResult>;validateWebhook(payload:unknown):boolean;parseWebhook(payload:unknown):ParsedPaymentWebhook;}

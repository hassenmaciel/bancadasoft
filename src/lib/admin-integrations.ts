...
export async function listAdminIntegrations() {
  ...
  return {mode:settings?.providerMode??"TEST",providers:providers.filter(provider=>isProductionProviderCode(provider.code)).map((provider) => ({
    ...
    products: provider.products.map((link) => ({
      ...
      automationClass:link.automationClass,
      technicalEligibility:link.technicalEligibility,
      homologationStatus:link.homologationStatus,
      contractSignature:link.contractSignature,
      expectedDeliveryType:link.expectedDeliveryType,

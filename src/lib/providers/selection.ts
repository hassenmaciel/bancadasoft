...
export function resolveProviderProduct<T extends ProviderProductCandidate>(candidates:T[], mode:ProviderModeValue):ProviderResolution<T> {
  const eligible=candidates.filter(item=>item.mode===mode&&item.active&&item.provider.active&&(item.technicalEligibility===undefined||item.technicalEligibility==="READY")&&(mode!=="REAL"||isProductionProviderCode(item.provider.code)));

...
export default async function IntegrationsPage() {
  const [providers, heartGateway] = await Promise.all([listAdminIntegrations(), heartUnlocksGatewayStatus()]);
  ...
  return <><header className="admin-heading"><div><small>PROVIDERS</small><h1>Integrações</h1><p>Fornecedores e vínculos técnicos do catálogo.</p></div></header>
    ...
    <section className="integration-list">{providers.map((provider) => <article className="integration-card" key={provider.id}>
      ...
      {provider.code === "heartunlocks" && <div className="integration-note"><b>Gateway {heartGateway.configured ? "configurado" : "não configurado"}</b><span>{heartGateway.configured ? heartGateway.online ? "Gateway online" : "Gateway offline" : "Configure as variáveis do gateway para habilitar o health."}{heartGateway.checkedAt ? ` · Verificado em ${new Intl.DateTimeFormat("pt-BR",{dateStyle:"short",timeStyle:"medium"}).format(new Date(heartGateway.checkedAt))}` : ""}</span></div>}
    </article>)}</section>

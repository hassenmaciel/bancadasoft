...
export default function OrderList({ initialOrders, initialAttention = null, attentionLabel = null, orderStatuses, paymentStatuses, fulfillmentStatuses }: { initialOrders: AdminOrderListDTO[]; initialAttention?: string | null; attentionLabel?: string | null; orderStatuses: string[]; paymentStatuses: string[]; fulfillmentStatuses: string[] }) {
  ...
  }, [isDefault, query, orderStatus, paymentStatus, fulfillmentStatus, initialAttention]);
  ...
  return <><header className="admin-heading"><div><small>OPERAÇÃO</small><h1>Pedidos</h1><p>Acompanhe pagamentos, entregas e o histórico operacional.</p></div></header>
    ...
    {initialAttention&&attentionLabel&&<p className="attention-filter" role="status">Filtro ativo: <b>{attentionLabel}</b> <Link href="/admin/pedidos">Limpar filtro</Link></p>}
